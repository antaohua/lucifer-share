...
public class ShareMarketApiClient {
    ...
    @Autowired
    public ShareMarketApiClient(@Qualifier("httpClient") LuciferHttpClient httpClient,@Qualifier("autoProxyHttpClient") LuciferHttpClient autoProxyHttpClient, ApplicationEventPublisher publisher, TradingDayService tradingDayService) {
        ...
        for (int i = 0; i < 55; i++) {
            ...
            autoProxyHttpClient.getAsync("https://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeData?page=" + page + "&num=" + pageSize + "&sort=symbol&asc=1&node=hs_a&symbol=&_s_r_a=page", new LuciferHttpCallback() {
                ...
                public void onSuccess(String response) {
                    ...
                    if(CollectionUtils.isEmpty(stockMarketItemList)){
                        return;
                    }
                    stockMarketItemList.forEach(publisher::publishEvent);
                }
