...
@Service
@Slf4j
public class TdConceptMarketService extends BaseShardingService<TdConceptMarketMapper, BoardMarketEntity> implements ScheduledInterface {
    /**
    ...
    public List<String> getConceptBoardCodes(){
        if(CollectionUtils.isEmpty(CONCEPT_BOARD_CODES)){
            synchronized (cacheLock){
                if(CollectionUtils.isEmpty(CONCEPT_BOARD_CODES)){
                    try {
                        List<BoardMarketEntity> boardList = akToolsService.stockBoardConceptNameEm();
    ...
    private void updateConceptBoardCodes(List<BoardMarketEntity> conceptMarketEntities) {
        if (CollectionUtils.isEmpty(conceptMarketEntities)) {
            return;
        }
        Set<String> newCodes = conceptMarketEntities.stream().map(BoardMarketEntity::getBoardCode).collect(Collectors.toSet());
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void saveConceptBoardData(List<BoardMarketEntity> boardList) {
        if (CollectionUtils.isEmpty(boardList)) {
            return;
        }
     ...
     * @param boardList 概念板块数据列表
     */
    private void updateMarketDataCache(List<BoardMarketEntity> boardList) {
        if (CollectionUtils.isEmpty(boardList)) {
            return;
        }
        try {
            ...
            // 按概念板块代码分组存储
            for (BoardMarketEntity board : boardList) {
     ...
     * @return 当日市场数据列表
     */
    public List<BoardMarketEntity> getBoardMarketData(String boardCode) {
        ...
        if (dataSet != null && !dataSet.isEmpty()) {
            ...
            return dataSet.stream()
                    .map(obj -> JSON.parseObject(JSON.toJSONString(obj), BoardMarketEntity.class))
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public List<BoardMarketEntity> getRealtimeDataByCodeAndDate(String boardCode, Integer date) {
        ...
        try {
            ...
            if (dataSet != null && !dataSet.isEmpty()) {
                ...
                return dataSet.stream()
                    .map(obj -> JSON.parseObject(JSON.toJSONString(obj), BoardMarketEntity.class))
                    .sorted(Comparator.comparing(BoardMarketEntity::getUpdateTime))
                    .collect(Collectors.toList());
            }
            // 缓存未命中，从数据库查询
            // 解析日期
