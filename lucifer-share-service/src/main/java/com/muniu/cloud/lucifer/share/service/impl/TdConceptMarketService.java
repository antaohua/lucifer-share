...
@Service
@Slf4j
public class TdConceptMarketService extends BaseShardingService<TdConceptMarketMapper, TradeBoardMarketEntity> implements ScheduledInterface {
    /**
    ...
    private void updateConceptBoardCodes(List<TradeBoardMarketEntity> conceptMarketEntities) {
        if (CollectionUtils.isEmpty(conceptMarketEntities)) {
            return;
        }
        Set<String> newCodes = conceptMarketEntities.stream().map(TradeBoardMarketEntity::getBoardCode).collect(Collectors.toSet());
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void saveConceptBoardData(List<TradeBoardMarketEntity> boardList) {
        if (CollectionUtils.isEmpty(boardList)) {
            return;
        }
     ...
     * @param boardList 概念板块数据列表
     */
    private void updateMarketDataCache(List<TradeBoardMarketEntity> boardList) {
        if (CollectionUtils.isEmpty(boardList)) {
            return;
        }
        try {
            ...
            // 按概念板块代码分组存储
            for (TradeBoardMarketEntity board : boardList) {
     ...
     * @return 当日市场数据列表
     */
    public List<TradeBoardMarketEntity> getBoardMarketData(String boardCode) {
        ...
        if (dataSet != null && !dataSet.isEmpty()) {
            ...
            return dataSet.stream()
                    .map(obj -> JSON.parseObject(JSON.toJSONString(obj), TradeBoardMarketEntity.class))
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public List<TradeBoardMarketEntity> getRealtimeDataByCodeAndDate(String boardCode, Integer date) {
        ...
        try {
            ...
            if (dataSet != null && !dataSet.isEmpty()) {
                ...
                return dataSet.stream()
                    .map(obj -> JSON.parseObject(JSON.toJSONString(obj), TradeBoardMarketEntity.class))
                    .sorted(Comparator.comparing(TradeBoardMarketEntity::getUpdateTime))
                    .collect(Collectors.toList());
            }
            // 缓存未命中，从数据库查询
            // 解析日期
